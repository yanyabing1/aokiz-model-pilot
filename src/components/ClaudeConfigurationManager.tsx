...
如果一个问题的含义不清楚或事实上不连贯，请解释为什么而不是回答不正确的内容。如果你不知道问题的答案，请不要分享虚假信息。`,
  ...
  const [installSuccessTrigger, setInstallSuccessTrigger] = useState(0) // 用于触发 ClaudeCodeInstaller 的状态更新
    ...
    try {
      ...
      if (response.ok) {
        ...
        const legacyConfig: LegacyClaudeConfig = {
          model:
            claudeCodeSettings.env?.ANTHROPIC_MODEL ||
            claudeCodeSettings.model ||
    ...
    try {
      ...
      const claudeCodeSettings: ClaudeCodeSettings = {
        ...
        env: {
          ...
          // 确保关键环境变量存在
          ANTHROPIC_AUTH_TOKEN:
            config.env?.ANTHROPIC_AUTH_TOKEN || config.anthropic_auth_token || '',
          ANTHROPIC_BASE_URL:
            config.env?.ANTHROPIC_BASE_URL ||
            config.anthropic_base_url ||
            'https://open.bigmodel.cn/api/anthropic',
          ANTHROPIC_MODEL:
            config.env?.ANTHROPIC_MODEL || config.model || 'claude-3-5-sonnet-20241022',
          BASH_DEFAULT_TIMEOUT_MS:
            config.env?.BASH_DEFAULT_TIMEOUT_MS || String(config.timeout || 30000),
          CLAUDE_CODE_MAX_OUTPUT_TOKENS:
            config.env?.CLAUDE_CODE_MAX_OUTPUT_TOKENS || String(config.max_tokens || 1000000),
        },
      ...
      Object.keys(claudeCodeSettings.env || {}).forEach((key: string) => {
        if (
          ...
          (claudeCodeSettings.env[key as keyof typeof claudeCodeSettings.env] === undefined ||
            claudeCodeSettings.env[key as keyof typeof claudeCodeSettings.env] === null ||
            claudeCodeSettings.env[key as keyof typeof claudeCodeSettings.env] === '')
        ) {
    ...
    <div className="bg-dark min-h-screen font-inter text-light bg-grid overflow-x-hidden">
                ...
                {(!config.anthropic_base_url || !config.anthropic_auth_token) && (
                  <div className="mt-3 p-3 bg-yellow-500/10 border border-yellow-500/30 rounded-lg">
                    ...
                    <div className="text-yellow-300 text-xs space-y-1">
                      {!config.anthropic_base_url && (
                        <div>
                          • 缺少 ANTHROPIC_BASE_URL，建议设置为:
                          https://open.bigmodel.cn/api/anthropic
                        </div>
                      )}
                      {!config.anthropic_auth_token && (
                        <div>
                          • 缺少 ANTHROPIC_AUTH_TOKEN，请在基础设置中的API配置部分填入你的认证令牌
                        </div>
                      )}
                    </div>
                  </div>
                        ...
                        <div className="relative">
                          ...
                          >
                            <optgroup label="Claude 4 系列">
                              <option value="claude-4-20250514">Claude 4 - 最新模型</option>
                            </optgroup>
                        ...
                        <div>
                          ...
                          <div className="relative">
                            <input
                              ...
                              onChange={e =>
                                handleConfigChange('anthropic_auth_token', e.target.value)
                              }
      ...
      Model: ${config.model}`
                            ...
                            Modal.info({
                              title: '调试信息',
                              content: (
                                <div className="font-mono whitespace-pre-line text-sm">
                                  {debugInfo}
                                </div>
                              ),
                              okText: '确定',
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                          ...
                          <select
                            ...
                            onChange={e =>
                              handleConfigChange('permissions.defaultMode', e.target.value)
                            }
                          >
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                          ...
                          <p className="text-xs text-gray-400 mb-2">
                            Anthropic API 基础 URL，可用于自定义 API 端点
                          </p>
                          <input
                            type="text"
                            value={config.env?.ANTHROPIC_BASE_URL || ''}
                            onChange={e =>
                              handleConfigChange('env.ANTHROPIC_BASE_URL', e.target.value)
                            }
                            className="w-full bg-dark/50 border border-primary/40 rounded-lg py-3 px-4 text-light"
                          />
                        </div>
                        <div>
                          ...
                          <div className="relative">
                            <input
                              ...
                              onChange={e =>
                                handleConfigChange('env.ANTHROPIC_AUTH_TOKEN', e.target.value)
                              }
                              placeholder="输入你的认证令牌"
                              className="w-full bg-dark/50 border border-primary/40 rounded-lg py-3 px-4 pr-10 text-light focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary transition-all"
                            />
                        ...
                        <div>
                          ...
                          <input
                            ...
                            onChange={e =>
                              handleConfigChange('env.AWS_BEARER_TOKEN_BEDROCK', e.target.value)
                            }
                            className="w-full bg-dark/50 border border-primary/40 rounded-lg py-3 px-4 text-light"
                          />
                        </div>
                    ...
                    <div className="bg-dark/30 rounded-lg p-4">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                          ...
                          <p className="text-xs text-gray-400 mb-2">
                            默认使用的 Anthropic 模型名称
                          </p>
                          <input
                            type="text"
                            value={config.env?.ANTHROPIC_MODEL || ''}
                            onChange={e =>
                              handleConfigChange('env.ANTHROPIC_MODEL', e.target.value)
                            }
                            className="w-full bg-dark/50 border border-primary/40 rounded-lg py-3 px-4 text-light"
                          />
                        </div>
                        <div>
                          ...
                          <input
                            ...
                            onChange={e =>
                              handleConfigChange('env.ANTHROPIC_SMALL_FAST_MODEL', e.target.value)
                            }
                            className="w-full bg-dark/50 border border-primary/40 rounded-lg py-3 px-4 text-light"
                          />
                        </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                          ...
                          <input
                            ...
                            onChange={e =>
                              handleConfigChange('env.BASH_DEFAULT_TIMEOUT_MS', e.target.value)
                            }
                            className="w-full bg-dark/50 border border-primary/40 rounded-lg py-3 px-4 text-light"
                          />
                        </div>
                        <div>
                          ...
                          <input
                            ...
                            onChange={e =>
                              handleConfigChange('env.BASH_MAX_TIMEOUT_MS', e.target.value)
                            }
                            className="w-full bg-dark/50 border border-primary/40 rounded-lg py-3 px-4 text-light"
                          />
                        </div>
                        <div>
                          ...
                          <input
                            ...
                            onChange={e =>
                              handleConfigChange('env.BASH_MAX_OUTPUT_LENGTH', e.target.value)
                            }
                            className="w-full bg-dark/50 border border-primary/40 rounded-lg py-3 px-4 text-light"
                          />
                        </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                          ...
                          <input
                            ...
                            onChange={e =>
                              handleConfigChange('env.MAX_THINKING_TOKENS', e.target.value)
                            }
                            className="w-full bg-dark/50 border border-primary/40 rounded-lg py-3 px-4 text-light"
                          />
                        </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                          ...
                          <input
                            ...
                            onChange={e =>
                              handleConfigChange('env.MCP_TOOL_TIMEOUT', e.target.value)
                            }
                            className="w-full bg-dark/50 border border-primary/40 rounded-lg py-3 px-4 text-light"
                          />
                        </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="flex items-center justify-between">
                          ...
                          <input
                            type="checkbox"
                            checked={
                              config.env?.DISABLE_TELEMETRY === '1' ||
                              config.env?.DISABLE_TELEMETRY === 'true'
                            }
                            onChange={e =>
                              handleConfigChange(
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <input
                            ...
                            checked={
                              ...
                              config.env?.DISABLE_AUTOUPDATER === 'true'
                            }
                            onChange={e =>
                              handleConfigChange(
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <input
                            type="checkbox"
                            checked={
                              config.env?.DISABLE_ERROR_REPORTING === '1' ||
                              config.env?.DISABLE_ERROR_REPORTING === 'true'
                            }
                            onChange={e =>
                              handleConfigChange(
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <input
                            type="checkbox"
                            checked={
                              config.env?.DISABLE_COST_WARNINGS === '1' ||
                              config.env?.DISABLE_COST_WARNINGS === 'true'
                            }
                            onChange={e =>
                              handleConfigChange(
                ...
                <div className="glass-dark rounded-2xl p-6 border border-primary/30 shadow-glow">
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="bg-dark/30 rounded-lg p-4">
                      ...
                      <div className="space-y-3">
                        {Object.entries(config.hooks?.PreToolUse || {}).map(
                          ([tool, command], index) => (
                            <div key={index} className="flex items-center gap-3">
                              <select
                                value={tool}
                                onChange={e => {
                                  const newHooks = {
                                    ...((config.hooks?.PreToolUse || {}) as Record<string, any>),
                                  }
                    ...
                    <div className="bg-dark/30 rounded-lg p-4">
                      ...
                      <div className="space-y-3">
                        {Object.entries(config.hooks?.PostToolUse || {}).map(
                          ([tool, command], index) => (
                            <div key={index} className="flex items-center gap-3">
                              <select
                                value={tool}
                                onChange={e => {
                                  const newHooks = {
                                    ...((config.hooks?.PostToolUse || {}) as Record<string, any>),
                                  }
                                  delete newHooks[tool as string]
                                  newHooks[e.target.value] = command
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                          ...
                          <input
                            type="text"
                            value={
                              typeof config.hooks?.Notification === 'object'
                                ? JSON.stringify(config.hooks?.Notification)
                        ...
                        <div>
                          ...
                          <input
                            type="text"
                            value={
                              typeof config.hooks?.Stop === 'object'
                                ? JSON.stringify(config.hooks?.Stop)
                                : config.hooks?.Stop || ''
                            }
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div className="flex items-center justify-between">
                          ...
                          <input
                            ...
                            onChange={e =>
                              handleConfigChange('enableAllProjectMcpServers', e.target.checked)
                            }
                            className="toggle"
                          />
                        </div>
                    ...
                    <div className="p-6 overflow-y-auto max-h-[60vh]">
                      ...
                      <div className="mt-4 p-4 bg-yellow-500/10 border border-yellow-500/30 rounded-lg">
                        ...
                        <ul className="text-sm text-gray-300 space-y-1">
                          <li>
                            • <strong>ANTHROPIC_BASE_URL:</strong>{' '}
                      ...
                      <button
                        onClick={() => {
                          navigator.clipboard.writeText(
                            JSON.stringify(configPreview || config, null, 2)
                          )
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <input
                        ...
                        onChange={e =>
                          handleConfigChange('editor_settings.word_wrap', e.target.checked)
                        }
                        className="toggle"
                      />
                    </div>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <input
                        ...
                        onChange={e =>
                          handleConfigChange(
                            'editor_settings.syntax_highlighting',
                            e.target.checked
                          )
                        }
                        className="toggle"
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <input
                        ...
                        onChange={e =>
                          handleConfigChange('editor_settings.minimap', e.target.checked)
                        }
                        className="toggle"
                      />
                    </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <div>
                          ...
                          <select
                            ...
                            onChange={e =>
                              handleConfigChange(
                                'ui_settings.preferredNotifChannel',
                                e.target.value
                              )
                            }
                          >
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <input
                            ...
                            onChange={e =>
                              handleConfigChange('ui_settings.autoUpdates', e.target.checked)
                            }
                            className="toggle"
                          />
                        </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div className="flex items-center justify-between">
                          ...
                          <input
                            ...
                            onChange={e =>
                              handleConfigChange('ui_settings.verbose', e.target.checked)
                            }
                            className="toggle"
                          />
                        </div>
                        <div>
                          ...
                          <input
                            ...
                            onChange={e =>
                              handleConfigChange('cleanupPeriodDays', parseInt(e.target.value))
                            }
                            className="w-full bg-dark/50 border border-primary/40 rounded-lg py-3 px-4 text-light"
                          />
                        </div>
                        <div className="flex items-center justify-between">
                          ...
                          <input
                            ...
                            onChange={e =>
                              handleConfigChange('includeCoAuthoredBy', e.target.checked)
                            }
                            className="toggle"
                          />
                        </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                          ...
                          <select
                            ...
                            onChange={e =>
                              handleConfigChange('ui_settings.interface_density', e.target.value)
                            }
                          >
                        ...
                        <div>
                          ...
                          <input
                            ...
                            onChange={e =>
                              handleConfigChange('ui_settings.accent_color', e.target.value)
                            }
                            className="w-full bg-dark/50 border border-primary/40 rounded-lg py-3 px-4 text-light"
                          />
                        </div>
                        <div>
                          ...
                          <input
                            ...
                            onChange={e =>
                              handleConfigChange('ui_settings.font_size', parseInt(e.target.value))
                            }
                            className="w-full bg-dark/50 border border-primary/40 rounded-lg py-3 px-4 text-light"
                          />
                        </div>
                        <div className="flex items-center justify-between">
                          ...
                          <input
                            ...
                            onChange={e =>
                              handleConfigChange('ui_settings.show_line_numbers', e.target.checked)
                            }
                            className="toggle"
                          />
                        </div>
                        <div className="flex items-center justify-between">
                          ...
                          <input
                            ...
                            onChange={e =>
                              handleConfigChange('ui_settings.show_minimap', e.target.checked)
                            }
                            className="toggle"
                          />
                        </div>
                        <div className="flex items-center justify-between">
                          ...
                          <input
                            ...
                            onChange={e =>
                              handleConfigChange('ui_settings.word_wrap', e.target.checked)
                            }
                            className="toggle"
                          />
                        </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div className="flex items-center justify-between">
                          ...
                          <input
                            ...
                            onChange={e =>
                              handleConfigChange(
                                'ui_settings.notifications.enabled',
                                e.target.checked
                              )
                            }
                            className="toggle"
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <input
                            ...
                            onChange={e =>
                              handleConfigChange(
                                'ui_settings.notifications.sound',
                                e.target.checked
                              )
                            }
                            className="toggle"
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <input
                            ...
                            onChange={e =>
                              handleConfigChange(
                                'ui_settings.notifications.desktop',
                                e.target.checked
                              )
                            }
                            className="toggle"
          ...
          {/* 右侧列 */}
          <div className="lg:col-span-1 mb-6 glass-dark rounded-2xl">
              ...
              <div className="glass-dark rounded-2xl p-6 border border-primary/30 shadow-glow sticky top-8">
                ...
                {configStatus === 'error' && (
                  <div className="bg-red-500/10 border border-red-500/30 rounded-lg p-3">
                    ...
                    <div className="text-red-400 text-xs mb-2">无法加载配置文件</div>
                  </div>
                )}
                ...
                <div className="mt-4 pt-4 border-t border-gray-700">
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      ...
                      >
                        <i
                          className={`fas ${config.anthropic_auth_token ? 'fa-check' : 'fa-times'} mr-1`}
                        ></i>
                        {config.anthropic_auth_token ? '已设置' : '未设置'}
                      </div>
                    </div>
                    <div className="flex items-center justify-between">
                      ...
                      >
                        <i
                          className={`fas ${config.anthropic_base_url ? 'fa-check' : 'fa-times'} mr-1`}
                        ></i>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      >
                        <i
                          className={`fas ${config.system_prompt ? 'fa-check' : 'fa-exclamation'} mr-1`}
                        ></i>
      ...
      {showInstallModal && (
        <ClaudeInstallModal
          ...
          onInstallSuccess={() => {
            ...
            // 重新检查安装状态
            checkClaudeCodeInstallStatus()
            // 触发 ClaudeCodeInstaller 的状态更新
            setInstallSuccessTrigger(prev => prev + 1)
          }}
